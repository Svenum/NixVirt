  ...
  process = with generate;
    elem "domain" [ (subattr "type" typeString) ]
      [
        ...
        (sub "qemu-commandline" (elem "qemu:commandline"
          [ (attr "xmlns:qemu" (typeConstant "http://libvirt.org/schemas/domain/qemu/1.0")) ]
          [
            (subelem "qemu:arg" [ (subattr "value" typeString) ] [ ])
            (subelem "qemu:env" [ (subattr "name" typeString) (subattr "value" typeString) ] [ ])
          ]))
      ];
